...
def generate_pristine_graphene_hexagon(n_r, filename1):
    
    import math
    n = int(n_r)
    coord0 = [0, 0, 0]
    list_of_coords = []
    a = 2 * 1.418 * math.cos(math.pi / 6)
...
def generate_pristine_graphene_rectangle(n, m, k, filename1):
...
def add_edge_hydrogens(atom_list):
    ...
    for atom in candidates:
        ...
        hx, hy, hz = points_x[idx], points_y[idx], points_z[idx]
...
def add_hydrogens_to_edges(init_file, filename1):
    """
    Интерфейсная функция для вызова из командной строки.
    Читает PDB-файл, добавляет водороды по краям и записывает результат.
    """
    
    atom_list = read_in_GO(init_file)

    hydgorened_atom_list = add_edge_hydrogens(atom_list)
...
#MY PART'S START
    ...
    no_OH_int = 2 * random.randint(0, no_OH // 2)
    
    internal_target = min(max_internal, no_OH_int)
    edge_target = no_OH - internal_target

    print(f"Internal hydroxyl groups - {internal_target}")
    print(f"Edge hydroxyl groups - {edge_target}")
...
#MY PART'S START
    ...
    internal_atoms_copy = internal_available.copy()
    internal_added = 0
    while internal_added < internal_target and internal_atoms_copy:
        atom = random.choice(internal_atoms_copy)
        old_len = len(atoms)
        new_atoms = add_hydroxyl(atom, atoms, added_functional_groups, top_or_down())
        if len(new_atoms) != old_len:
            atoms = new_atoms
            added_functional_groups += 1
            internal_added += 1
            internal_atoms_copy.remove(atom)
        else:
            internal_atoms_copy.remove(atom)

    edge_atoms_copy = edge_map.copy()
    edge_added = 0
    while edge_added < edge_target and edge_atoms_copy:
        atom = random.choice(edge_atoms_copy)
        old_len = len(atoms)
        new_atoms = add_hydroxyl(atom, atoms, added_functional_groups, top_or_down())
        if len(new_atoms) != old_len:
            atoms = new_atoms
...
def writepdb(list_of_atoms, filename1):
    """PDB writer"""
    if ((".pdb" not in filename1) and (".PDB" not in filename1)):
        string = str(filename1) + ".pdb"
    else:
...
### MY PART'S START ###
        ...
        try:
            generate_pristine_graphene_hexagon(str(sys.argv[2]), str(sys.argv[3]))
        except:
            print("Please type 'python GOPY.py help'")
    elif (sys.argv[1] == "generate_PG_rectangle"):
        try:
...
'python GOPY.py generate_N_doped path_to_file 10 9 8 file_to_save'              - Used to generate an N-doped graphene layer. X Y and Z represent the number of N-graphitic,
                                                                               ...
                                                                               E.g. 'python GOPY.py /path/to/PG.pdb 10 9 8 Ndoped.pdb' generates an N-doped molecule of the 
